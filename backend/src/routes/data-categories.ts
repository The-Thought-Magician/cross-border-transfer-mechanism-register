...
import { data_categories, workspaces, workspace_members, audit_logs } from '../db/schema.js'
...
async function log(workspaceId: string | null, userId: string, action: string, entityId: string, detail: Record<string, unknown>) {
  if (!workspaceId) return // global catalog rows aren't tied to a workspace audit log
  try {
...
const createSchema = z.object({
  ...
  sensitivity_weight: z.number().int().min(1).max(5).optional(),
...
router.get('/', async (c) => {
  ...
  if (globalOnly) {
    rows = await db
      ...
      .where(isNull(data_categories.workspace_id))
      .orderBy(desc(data_categories.sensitivity_weight))
  } else if (workspaceId) {
    // global rows (workspace_id IS NULL) plus this workspace's rows
    const all = await db.select().from(data_categories).orderBy(desc(data_categories.sensitivity_weight))
    rows = all.filter((r) => r.workspace_id === null || r.workspace_id === workspaceId)
  } else {
    rows = await db.select().from(data_categories).orderBy(desc(data_categories.sensitivity_weight))
  }
...
router.post('/', authMiddleware, zValidator('json', createSchema), async (c) => {
  ...
  // If scoped to a workspace, the caller must be a member.
  if (body.workspace_id && !(await isWorkspaceMember(body.workspace_id, userId))) {
    return c.json({ error: 'Forbidden' }, 403)
  }

  const [created] = await db
    .insert(data_categories)
    .values({
      workspace_id: body.workspace_id ?? null,
      name: body.name,
      article: body.article ?? null,
      sensitivity_weight: body.sensitivity_weight ?? 1,
...
router.put('/:id', authMiddleware, zValidator('json', updateSchema), async (c) => {
  ...
  const [existing] = await db.select().from(data_categories).where(eq(data_categories.id, id))
  if (!existing) return c.json({ error: 'Not found' }, 404)

  // Workspace-scoped rows require membership; global rows are not editable here.
  ...
  const [updated] = await db
    ...
    .set({
      ...
      article: body.article ?? existing.article,
      sensitivity_weight: body.sensitivity_weight ?? existing.sensitivity_weight,
      description: body.description ?? existing.description,
      is_special: body.is_special ?? existing.is_special,
    })
    .where(eq(data_categories.id, id))
    .returning()
...
router.delete('/:id', authMiddleware, async (c) => {
  ...
  await db.delete(data_categories).where(eq(data_categories.id, id))
